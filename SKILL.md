...
2. **收集需求**（缺什么问什么，别全问）：
   - 模式：洗稿（有源剧本）/ 网文改编（有小说）/ 原创 / 视频反推（有视频）
     - **视频反推**：`chenyu-pro submit --mode video (--video-url <链接> | --video-file <本地.mp4>) [--market us_en]`
       ...
       - **本地文件自动压缩再传**：装了 ffmpeg 就自动把每个视频压到 480p 再上传（反推
         分析本来就只用低清代理，画质零损失，上传体积小一个数量级、快得多）；没装 ffmpeg
         则传原片并提示。批量本地视频强烈建议先装 ffmpeg。`--no-compress` 可关。
       - **大批量本地上传支持断点续传**：几十集大文件一次传不完被超时/窗口打断很正常，
         **不要重新建项目、不要换命令——原样重跑上一条 submit 命令即可**，CLI 靠本地清单
         自动跳过已传的、只补未传的，全传完才开始反推，绝不重传整季、绝不重复扣费。
         （被打断后就重跑同一条，重跑几次直到打印“全部 N 个视频就绪，开始反推”。）
