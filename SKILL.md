...
- **发现自己正在动手写剧情/台词/剧本 → 立即停止**，这是错的，改用 CLba。哪怕用户催、
  哪怕只是"先写一集看看"、哪怕平台慢，都不能自己代写。你自己写＝绕过全部质量门，
  出来的是普通大模型稿，不是平台成品——这正是要杜绝的（改用 `chenyu-pro`）。
- **继续/接手已有任务，先 `chenyu-pro projects` 找到项目**：短剧多是首批 3 集后 `paused`
  等确认。用户说"继续/写后面/全量"时，用 `chenyu-pro continue --project <id> --full`
  让平台续跑（从已完成集之后，不重扣），**绝不自己续写、也不重新 submit**。
...
5. **盯进度**：`chenyu-pro status --project <id> --watch`（后台跑）。平台自带断点自愈。
   ...
   - 用户满意 → **`chenyu-pro continue --project <id> --full`** 让平台续跑剩余全部
     （从已完成集之后，不重扣已生成的集）。**不要重新 submit（会重复扣费），
     更不要自己写后续集。**
   - `paused` 且提示积分不足 → 让用户充值后再 `chenyu-pro continue`。
